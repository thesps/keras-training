...
def three_layer_model_batch_norm_double(Inputs, nclasses, l1Reg=0):
    """
    Two hidden layers model
    """
    x = Dense(128, kernel_initializer='lecun_uniform',
              name='fc1_relu', W_regularizer=l1(l1Reg))(Inputs)
    x = BatchNormalization(epsilon=1e-6, momentum=0.9, name='bn1')(x)
    x = Activation(activation='relu', name='relu1')(x)
...
def three_layer_model_binary_relu(Inputs, nclasses, l1Reg=0):
    """
    Three hidden layers model
    """
    ...
    model.add(BinaryDense(64, H='Glorot', kernel_lr_multiplier='Glorot', use_bias=False, name='fc1', input_shape=(16,)))
    model.add(BatchNormalization(epsilon=1e-6, momentum=0.9, name='bn1'))
    model.add(Activation('relu', name='relu{}'.format(1)))

    model.add(BinaryDense(32, H='Glorot', kernel_lr_multiplier='Glorot', use_bias=False, name='fc2'))
    model.add(BatchNormalization(epsilon=1e-6, momentum=0.9, name='bn2'))
    model.add(Activation('relu', name='relu{}'.format(2)))

    model.add(BinaryDense(32, H='Glorot', kernel_lr_multiplier='Glorot', use_bias=False, name='fc3'))
    model.add(BatchNormalization(epsilon=1e-6, momentum=0.9, name='bn3'))
    model.add(Activation('relu', name='relu{}'.format(3)))
...
def three_layer_model_binary_relu_double(Inputs, nclasses, l1Reg=0):
    """
    Three hidden layers model
    """
    ...
    model.add(BinaryDense(128, H=1, kernel_lr_multiplier='Glorot', use_bias=False, name='fc1', input_shape=(16,)))
    model.add(BatchNormalization(epsilon=1e-6, momentum=0.9, name='bn1'))
    model.add(Activation('relu', name='relu{}'.format(1)))

    model.add(BinaryDense(64, H=1, kernel_lr_multiplier='Glorot', use_bias=False, name='fc2'))
    model.add(BatchNormalization(epsilon=1e-6, momentum=0.9, name='bn2'))
    model.add(Activation('relu', name='relu{}'.format(2)))

    model.add(BinaryDense(64, H=1, kernel_lr_multiplier='Glorot', use_bias=False, name='fc3'))
    model.add(BatchNormalization(epsilon=1e-6, momentum=0.9, name='bn3'))
    model.add(Activation('relu', name='relu{}'.format(3)))

    model.add(BinaryDense(nclasses, H=1, kernel_lr_multiplier='Glorot', use_bias=False, name='output'))
    model.add(BatchNormalization(epsilon=1e-6, momentum=0.9, name='bn'))

    return model

def five_layer_model_binary(Inputs, nclasses, l1Reg=0):
    """
    Three hidden layers model
    """
    ...
    model.add(BinaryDense(128, H='Glorot', kernel_lr_multiplier='Glorot', use_bias=False, name='fc1', input_shape=(16,)))
    model.add(BatchNormalization(epsilon=1e-6, momentum=0.9, name='bn1'))
    model.add(Activation(binary_tanh, name='act{}'.format(1)))

    model.add(BinaryDense(64, H='Glorot', kernel_lr_multiplier='Glorot', use_bias=False, name='fc2'))
    model.add(BatchNormalization(epsilon=1e-6, momentum=0.9, name='bn2'))
    model.add(Activation(binary_tanh, name='act{}'.format(2)))

    model.add(BinaryDense(64, H='Glorot', kernel_lr_multiplier='Glorot', use_bias=False, name='fc3'))
    model.add(BatchNormalization(epsilon=1e-6, momentum=0.9, name='bn3'))
    model.add(Activation(binary_tanh, name='act{}'.format(3)))

    model.add(BinaryDense(64, H='Glorot', kernel_lr_multiplier='Glorot', use_bias=False, name='fc4'))
...
def five_layer_model_constraint(Inputs, nclasses, l1Reg=0, h5fName=None):
    """
    Two hidden layers model
    """
    h5f = h5py.File(h5fName)
    x = Dense(128, activation='relu', kernel_initializer='lecun_uniform',
              name='fc1_relu', W_regularizer=l1(l1Reg),
              kernel_constraint = zero_some_weights(binary_tensor=h5f['fc1_relu'][()].tolist()))(Inputs)
    x = Dense(64, activation='relu', kernel_initializer='lecun_uniform',
              name='fc2_relu', W_regularizer=l1(l1Reg),
    ...
    x = Dense(64, activation='relu', kernel_initializer='lecun_uniform',
              ...
              kernel_constraint = zero_some_weights(binary_tensor=h5f['fc3_relu'][()].tolist()))(x)
    x = Dense(64, activation='relu', kernel_initializer='lecun_uniform',
              name='fc4_relu', W_regularizer=l1(l1Reg),
              kernel_constraint = zero_some_weights(binary_tensor=h5f['fc4_relu'][()].tolist()))(x)
    predictions = Dense(nclasses, activation='softmax', kernel_initializer='lecun_uniform',
                        name='output_softmax', W_regularizer=l1(l1Reg),
                        kernel_constraint = zero_some_weights(binary_tensor=h5f['output_softmax'][()].tolist()))(x)
    model = Model(inputs=Inputs, outputs=predictions)
