...
from tensorflow.keras.layers import Dense, Dropout, Flatten, Convolution2D, Convolution1D, Conv2D, Conv1D, Input, SpatialDropout1D, GRU, MaxPooling1D, AveragePooling1D, SimpleRNN, LSTM, BatchNormalization, Activation
...
def dense_model(Inputs, nclasses, l1Reg=0, dropoutRate=0.25):
    ...
    """
    x = Dense(200, activation='relu', kernel_initializer='lecun_uniform', name='fc1_relu', kernel_regularizer=l1(l1Reg))(Inputs)
    x = Dropout(dropoutRate)(x)
    x = Dense(200, activation='relu', kernel_initializer='lecun_uniform', name='fc2_relu', kernel_regularizer=l1(l1Reg))(x)
    x = Dropout(dropoutRate)(x)
    x = Dense(200, activation='relu', kernel_initializer='lecun_uniform', name='fc3_relu', kernel_regularizer=l1(l1Reg))(x)
    x = Dropout(dropoutRate)(x)
    x = Dense(200, activation='relu', kernel_initializer='lecun_uniform', name='fc4_relu', kernel_regularizer=l1(l1Reg))(x)
    x = Dropout(dropoutRate)(x)
    x = Dense(200, activation='relu', kernel_initializer='lecun_uniform', name='fc5_relu', kernel_regularizer=l1(l1Reg))(x)
...
def two_layer_model_constraint(Inputs, nclasses, l1Reg=0, h5fName=None):
    ...
    predictions = Dense(nclasses, activation='sigmoid', kernel_initializer='lecun_uniform', 
                        name = 'output_sigmoid', kernel_regularizer=l1(l1Reg), 
...
def three_layer_model_apfixed(Inputs, nclasses, l1Reg=0):
    """
    Three hidden layers model
    """
    W, I = 4, 0    
    ...
    model.add(QDense(64,
                     kernel_quantizer = quantized_bits(W,I,0),
                     bias_quantizer   = quantized_bits(W,I,0),
                     name             = 'fc1',
                     input_shape      = (16,)))

    model.add(BatchNormalization(epsilon=1e-6, momentum=0.9, name='bn1'))
    model.add(QActivation(quantized_relu(W), name='act{}'.format(1)))     
    ...
    model.add(BatchNormalization(epsilon=1e-6, momentum=0.9, name='bn2'))
    model.add(QActivation(quantized_relu(W), name='act{}'.format(2)))     
    ...
    model.add(BatchNormalization(epsilon=1e-6, momentum=0.9, name='bn3'))
    model.add(QActivation(quantized_relu(W), name='act{}'.format(3)))     
    
    model.add(QDense(nclasses,
                     kernel_quantizer = quantized_bits(W,I,0),
                     bias_quantizer   = quantized_bits(W,I,0),
                     name             = 'output'))
...
def three_layer_model_ternary(Inputs, nclasses, l1Reg=0):
    ...
    model.add(QDense(64, kernel_quantizer=ternary(), use_bias=False, name='fc1', input_shape=(16,)))
    model.add(BatchNormalization(epsilon=1e-6, momentum=0.9, name='bn1'))
    model.add(QActivation(quantized_tanh(2,0,1), name='act{}'.format(1)))
    
    model.add(QDense(32, kernel_quantizer=ternary(), use_bias=False, name='fc2'))
    model.add(BatchNormalization(epsilon=1e-6, momentum=0.9, name='bn2'))
    model.add(QActivation(quantized_tanh(2,0,1), name='act{}'.format(2)))  
    
    model.add(QDense(32, kernel_quantizer=ternary(), use_bias=False, name='fc3'))
    model.add(BatchNormalization(epsilon=1e-6, momentum=0.9, name='bn3'))
    model.add(QActivation(quantized_tanh(2,0,1), name='act{}'.format(3)))  
        
    model.add(QDense(nclasses, kernel_quantizer=ternary(), use_bias=False, name='output'))
    model.add(BatchNormalization(epsilon=1e-6, momentum=0.9, name='bn'))
...
def three_layer_model_constraint(Inputs, nclasses, l1Reg=0, h5fName=None):
    ...
    x = Dense(64, activation='relu', kernel_initializer='lecun_uniform', 
              ...
              kernel_constraint = zero_some_weights(binary_tensor=h5f['fc1_relu'][()].tolist()))(Inputs)
    x = Dense(32, activation='relu', kernel_initializer='lecun_uniform', 
              name='fc2_relu', kernel_regularizer=l1(l1Reg), 
              kernel_constraint = zero_some_weights(binary_tensor=h5f['fc2_relu'][()].tolist()))(x)
    x = Dense(32, activation='relu', kernel_initializer='lecun_uniform', 
              name='fc3_relu', kernel_regularizer=l1(l1Reg), 
...
def claudionor(Inputs, nclasses, l1Reg=0):
    """
    Two hidden layers model
    """
...
def claudionor_three_layer_model_batch_norm(Inputs, nclasses, l1Reg=0):
    """
    Two hidden layers model
    """
    q_dict = {
